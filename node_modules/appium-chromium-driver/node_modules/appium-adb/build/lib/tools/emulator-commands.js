...
async function listEmulators() {
    ...
    if (await (0, helpers_1.dirExists)(avdsRoot ?? '')) {
        return await getAvdConfigPaths(/** @type {string} */ (avdsRoot));
    }
    if (avdsRoot) {
        logger_js_1.log.warn(`The value of the ANDROID_AVD_HOME environment variable '${avdsRoot}' is not an existing directory`);
    }
    const prefsRoot = await (0, helpers_1.getAndroidPrefsRoot)();
    if (!prefsRoot) {
        return [];
    }
    avdsRoot = path_1.default.resolve(prefsRoot, 'avd');
    if (!await (0, helpers_1.dirExists)(avdsRoot)) {
        logger_js_1.log.debug(`Virtual devices config root '${avdsRoot}' is not an existing directory`);
        return [];
    }
...
async function getAvdConfigPaths(avdsRoot) {
    ...
    return configs.map((confPath) => {
        const avdName = path_1.default.basename(confPath).split('.').slice(0, -1).join('.');
...
async function sensorSet(sensor, value) {
    if (!lodash_1.default.includes(this.SENSORS, sensor)) {
        throw new TypeError(`Unsupported sensor sent '${sensor}'. `
            + `Supported values: ${lodash_1.default.values(this.SENSORS)}]`);
    }
...
async function gsmSignal(strength = 4) {
    ...
    logger_js_1.log.info('gsm signal-profile <strength> changes the reported strength on next (15s) update.');
...
async function execEmuConsoleCommand(cmd, opts = {}) {
    ...
    return await new bluebird_1.default((resolve, reject) => {
        const connTimeoutObj = setTimeout(() => reject(new Error(`Cannot connect to the Emulator console at ${host}:${port} ` +
        ...
        client.on('data', (chunk) => {
            ...
            if (okFlag.test(output)) {
                ...
                if (!isCommandSent) {
                    ...
                    logger_js_1.log.debug(`Executing Emulator console command: ${cmdStr}`);
                    client.write(cmdStr);
                    client.write(eol);
                    isCommandSent = true;
...
async function getEmuVersionInfo() {
    const propsPath = path_1.default.join(/** @type {string} */ (this.sdkRoot), 'emulator', 'source.properties');
