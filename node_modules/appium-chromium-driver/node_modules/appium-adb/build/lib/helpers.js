...
exports.extractMatchingPermissions = exports.getBuildToolsDirs = exports.getSdkToolsVersion = exports.getOpenSslForOs = exports.getJavaForOs = exports.getJavaHome = exports.getResourcePath = exports.DEFAULT_ADB_EXEC_TIMEOUT = exports.APKS_INSTALL_TIMEOUT = exports.APK_INSTALL_TIMEOUT = exports.APK_EXTENSION = exports.APKS_EXTENSION = void 0;
...
exports.isScreenStateOff = isScreenStateOff;
...
exports.APKS_INSTALL_TIMEOUT = exports.APK_INSTALL_TIMEOUT * 2;
...
exports.getSdkToolsVersion = lodash_1.default.memoize(async function getSdkToolsVersion() {
    ...
    if (!await support_1.fs.exists(propertiesPath)) {
        logger_js_1.log.warn(`Cannot find ${propertiesPath} file to read SDK version from`);
        return null;
    }
    ...
    logger_js_1.log.warn(`Cannot parse "Pkg.Revision" value from ${propertiesPath}`);
    return null;
});
/**
...
async function formatConfigMarker(configsGetter, desiredMarker, defaultMarker) {
    ...
    logger_js_1.log.debug(`Resource configurations: ${JSON.stringify(configs)}`);
    ...
    if (configMarker.toLowerCase().startsWith('en')
        ...
        logger_js_1.log.debug(`Resource configuration name '${configMarker}' is unknown. ` +
            `Replacing it with '${defaultMarker}'`);
        configMarker = defaultMarker;
    }
    else {
        logger_js_1.log.debug(`Selected configuration: '${configMarker}'`);
    }
    return configMarker;
}
/**
...
async function getAndroidPrefsRoot() {
    ...
    if (location) {
        logger_js_1.log.warn(`The value of the ANDROID_EMULATOR_HOME environment variable '${location}' is not an existing directory`);
    }
    ...
    if (!await dirExists(location ?? '')) {
        logger_js_1.log.debug(`Android config root '${location}' is not an existing directory`);
        return null;
    }
    return location ?? null;
}
