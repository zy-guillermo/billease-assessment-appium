/**
 * @this {import('../driver').AndroidDriver}
 * @returns {import('@appium/types').StringRecord<string>}
 */
export function mobileCommandsMapping(this: import("../driver").AndroidDriver): import("@appium/types").StringRecord<string>;
/**
 * @this {import('../driver').AndroidDriver}
 * @param {string} script
 * @param {import('@appium/types').StringRecord[]|import('@appium/types').StringRecord} [args]
 * @returns {Promise<any>}
 */
export function execute(this: import("../driver").AndroidDriver, script: string, args?: import("@appium/types").StringRecord[] | import("@appium/types").StringRecord): Promise<any>;
/**
 * @this {import('../driver').AndroidDriver}
 * @param {string} mobileCommand
 * @param {import('@appium/types').StringRecord} [opts={}]
 * @returns {Promise<any>}
 */
export function executeMobile(this: import("../driver").AndroidDriver, mobileCommand: string, opts?: import("@appium/types").StringRecord): Promise<any>;
//# sourceMappingURL=execute.d.ts.map